> ## ⚠️ DEPRECATED (v3)
>
> 本文件由 v3.1 判定为**不可靠**，原因如下：
> 1. 其依据的 `figures_v3/overlay_foreground.png` 使用**未对齐**的 `fg_world`（漏掉 Sim3 对齐），
>    而指标用的是已对齐 `P_fore`，导致视觉图与指标不一致。
> 2. 其依据的 `figures_v3/*_depth_montage.png` 实际显示的是 **validity mask（二值）**，而非真实深度值。
> 3. 其采用的 3DGS `splat.ply` 为 **pseudo-reference**，被错误地称作 ground-truth。
> 4. 未审计参考深度单位（实际为 uint16 毫米），导致 depth 结论不可信。
>
> 新判定见 `VISUAL_AUDIT_v31.md`（基于修复后的 aligned foreground overlay、真实 depth montage、
> 严格 5/10/20mm 指标、robust phenotype、scanner-GT 单 plant 验证）。
